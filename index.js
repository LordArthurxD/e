...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Lord Arthur ⁩⁩\n' 
            + 'ORG: Lord Arthur ⁩;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=16514190715+1 (651) 419-0715n' 
            + 'END:VCARD' 
blocked = []   
prefix = '#'
limitawal = 30
...
/******** OWNER NUMBER**********/
const ownerNumber = ["5491153261742@s.whatsapp.net","5491153251742@s.whatsapp.net"] 
/************************************/
...
• Limit: ${limitawal-updated.limit}`
           ...
           case 'demote':
				...
				case 'setppbot':
					if (!isOwner) return reply(ind.ownerb())
				    client.updatePresence(from, Presence.composing) 
					if (!isQuotedImage) return reply(`Responde a una imagen con el comando ${prefix}setbotpp`)
				...
				case 'bcgc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 ANUNCIO GROUPO 」*\n\nDel Grupo : ${groupName}\nRemitente : wa.me/${(sender.split('@')[0])}\nMensaje : ${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
